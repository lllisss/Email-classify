...
def load_positive_negative_data_files(positive_data_file, negative_data_file,new_data_file):
    """
    ...
    new_examples = read_and_clean_zh_file(new_data_file)
    # Combine data
    x_text = positive_examples + negative_examples+new_examples
    # Generate labels
    positive_labels = [[0, 0, 1] for _ in positive_examples]
    negative_labels = [[0, 1, 0] for _ in negative_examples]
    new_labels = [[1, 0, 0] for _ in new_examples]
    y = np.concatenate([positive_labels, negative_labels,new_labels], 0)
    return [x_text, y]
...
def seperate_line1(line):
    return str(jieba.lcut(line))

def read_and_clean_zh_file(input_file, output_cleaned_file = None):
    lines = list(open(input_file, "rb").readlines())
    lines = [clean_str(seperate_line1(line.decode('utf-8'))) for line in lines]
